# app to compute brushed hit rates
library(shiny)
library(ggplot2)
library(dplyr)
library(readr)
...
# merge two datasets
rbind(scip[, c(1:4, 7)], scip_2021, scip_2022,
      scip_2023)  %>%
  filter(game_year %in% 2015:2023) -> scip

# want H variable to be character
scip$H <- ifelse(scip$H == 1,
                  "YES", "NO")
...
ui <- fluidPage(
  ...
  fluidRow(
    column(4, wellPanel(
      h4(id="big-heading", "Hit Rates in
         Statcast Era"),
      dateInput("date_lo",
...
#     actionButton("goButton", "Update Table")
    ...
    column(8,
      ...
      of hard-hit ball and hit  rates over the Statcast era.  One selects a
      ...
      balls in play
      and the count and rate of hit from the selected region.
       By choosing the Graph of Rates tab, one sees a graph of the batted ball
        rates and hit rates plotted against season."),
